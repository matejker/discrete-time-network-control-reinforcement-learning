...
def test_sarsa_training(network_cycle_4):
    ...
    model = Sarsa(initial_state, end_state, network_cycle_4, input_matrix, episodes_factor=13, iteration_factor=2)
...
def test_4_step_sarsa_training(network_cycle_4):
    input_matrix = {0: 0}
    q = 4
    n = network_cycle_4.nodes
    ...
    model = Sarsa(
        initial_state, end_state, network_cycle_4, input_matrix, episodes_factor=13, iteration_factor=2, n_steps=4
    )

    assert model.__repr__() == "Sarsa(trained=not yet)"
    ...
    assert model.__repr__() == "Sarsa(trained=yes, time_horizon=3)"
